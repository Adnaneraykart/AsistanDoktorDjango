...
import xgboost as xgb  # XGBoost import edilmiştir
import os
...
def main():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X), 1):
        ...
        # LightGBM ve XGBoost tahminleri
        y_pred_lgb = lgb_model.predict(X_val)
        y_pred_xgb = xgb_model.predict(X_val)

        # Hibrit model tahmini (ağırlıklı ortalama)
        y_pred_hybrid = (y_pred_lgb + y_pred_xgb) / 2
        y_pred_hybrid = np.round(y_pred_hybrid)
        ...
        acc = accuracy_score(y_val, y_pred_hybrid)
        f1 = f1_score(y_val, y_pred_hybrid)
        auc = roc_auc_score(y_val, y_pred_hybrid)
        ...
        # ROC Eğrisini çizme
        fpr, tpr, _ = roc_curve(y_val, y_pred_hybrid)
        plt.figure(figsize=(8, 6))
        plt.plot(fpr, tpr, color='blue', label='ROC Eğrisi')
        plt.plot([0, 1], [0, 1], color='red', linestyle='--')
        plt.title(f'LightGBM + XGBoost')
    ...
    test_results = pd.DataFrame({
        ...
        'True Label': y_val,
        'Predicted Label': y_pred_hybrid
    })

    print("\nTest Verileri ve Hibrit Tahminler:")
    print(test_results)
    ...
    plt.savefig("test_results_table_hybrid.png", format="png", bbox_inches='tight', dpi=300)
    ...
    # Görselleştirmeyi bir dosyaya kaydet
    plt.savefig("shap_summary_plot_hybrid.png", format="png", bbox_inches="tight")  # PNG formatında kaydet
    plt.close()  # Görseli kapat
